...
router.put("/profile", authenticateToken, authorizeRoles("Dentist"), async (req, res) => {
  const user_id = req.user.user_id;
  const { name, email, license_number, specialization, availability } = req.body || {};

  if (!name || !email || !license_number || !specialization || !availability) {
    return res.status(400).json({
      error: "Name, email, license number, specialization, and availability are required",
    });
  }
  ...
  try {
    ...
    const updatedDentist = await client.query(
      `UPDATE public.dentists
       ...
       WHERE user_id = $4
       RETURNING *`,
      [license_number, specialization, availability, user_id]
    );
