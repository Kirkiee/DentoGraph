...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // If tooth_id is provided, check if tooth exists
      if (tooth_id) {
        const toothCheck = await pool.query(
          'SELECT tooth_id FROM teeth WHERE tooth_id = $1 AND record_id = $2',
          [tooth_id, record_id]
        );
    ...
    try {
      const xrays = await pool.query(
        `SELECT 
            ...
            xray_images.tooth_id,
            teeth.tooth_number,
            xray_images.file_path,
            xray_images.upload_date
         FROM xray_images
         LEFT JOIN teeth ON xray_images.tooth_id = teeth.tooth_id
         WHERE xray_images.record_id = $1
         ORDER BY xray_images.upload_date DESC`,
        [record_id]
      );
